...
export class ConnEnd {
  ...
  assignPinVisibilityTo(dummyConnectionVert: IVertInf, targetVert: IVertInf): void {
    ...
    for (const currPin of this.m_anchor_obj!.m_connection_pins as Set<ShapeConnectionPin>) {
      if (
        ...
        if (router.m_allows_orthogonal_routing && currPin.m_vertex) {
          const edge = new EdgeInf(dummyConnectionVert as any, currPin.m_vertex as any, true);
          edge.setDist(
        ...
        if (router.m_allows_polyline_routing && currPin.m_vertex) {
          const edge = new EdgeInf(dummyConnectionVert as any, currPin.m_vertex as any, false);
          edge.setDist(
  ...
  getHyperedgeVertex(router: IRouter): [boolean, IVertInf | null] {
    ...
    if (this.m_anchor_obj) {
      ...
      vertex = new VertInf(router as any, id, this.m_point) as any;
      vertex!.visDirections = this.m_directions;
      addedVertex = true;

      if (router.m_allows_polyline_routing && (router as any)._vertexVisibility) {
        (router as any)._vertexVisibility(vertex, null, true, true);
      }
    }
