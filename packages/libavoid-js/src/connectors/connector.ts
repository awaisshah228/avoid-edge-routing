...
export class ConnRef {
  ...
  setRoutingCheckpoints(checkpoints: Checkpoint[]): void {
    ...
    for (let i = 0; i < this._checkpoints.length; ++i) {
      const ptID = new VertID(this._id, 2 + i, PROP_ConnPoint | PROP_ConnCheckpoint);
      const vertex = new VertInf(this._router, ptID, this._checkpoints[i].point);
      this._checkpoint_vertices.push(vertex as any);
    }

    if (this._router.m_allows_polyline_routing) {
      for (let i = 0; i < this._checkpoints.length; ++i) {
        if (this._router._vertexVisibility) {
          this._router._vertexVisibility(this._checkpoint_vertices[i], null, true, true);
        }
      }
    }
  }
  ...
  setEndpointByVertID(type: number, pointID: VertIDLike, pointSuggestion: Point | null = null): boolean {
    ...
    const edge = new EdgeInf(
        (type === VertID_src) ? this._src_vert as any : this._dst_vert as any, vInf as any);
    edge.setDist(0.001);
  ...
  common_updateEndPoint(type: number, connEnd: any): void {
    ...
    const ptID = new VertID(this._id, type, properties);

    if (type === VertID_src) {
      if (this._src_vert) {
        this._src_vert.Reset(ptID, point);
      } else {
        this._src_vert = new VertInf(this._router, ptID, point) as any;
      }
      ...
      if (this._dst_vert) {
        ...
        this._dst_vert = new VertInf(this._router, ptID, point) as any;
      }
      if (isConnEnd && connEnd.directions) {
  ...
  updateEndPoint(type: number, connEnd: any): void {
    ...
    if (this._router.m_allows_polyline_routing) {
      ...
      if (type === VertID_src) {
        ...
        if (!dummySrc) {
          if (this._router._vertexVisibility) {
            this._router._vertexVisibility(this._src_vert, this._dst_vert, knownNew, genContains);
          }
        }
      } else {
        ...
        if (!dummyDst) {
          if (this._router._vertexVisibility) {
            this._router._vertexVisibility(this._dst_vert, this._src_vert, knownNew, genContains);
          }
        }
      }
    }
  ...
  generatePath(): boolean {
    ...
    for (let i = 1; i < vertices.length; ++i) {
      if (this._router.InvisibilityGrph && this._type === ConnType_PolyLine) {
        const existingEdge = EdgeInf.existingEdge(vertices[i-1] as any, vertices[i] as any);
        if (existingEdge) {
          existingEdge.addConn(this._reroute_flag_ptr);
        }
      } else {
  ...
  generateCheckpointsPath(): { path: Point[]; vertices: PlaceholderVert[] } {
    ...
    for (let i = 1; i < checkpoints.length; ++i) {
      ...
      if (this._router._AStarPath) {
        const aStar = new this._router._AStarPath();
